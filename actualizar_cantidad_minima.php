...
if($_POST["id"] != "" && $_POST["existencia"] != ""){
    ...
    if($existencia_actual->Cantidad_Minima != "" || $existencia_actual->Cantidad_Maxima != ""){
        ...
        $qry_inventario_producto = "update ds_tbl_cantidad_minima_producto set Cantidad_Minima = $existencia, Fecha_Actualiza = '{$fecha_actualizacion}' where Id_Producto_Detalle = $id_producto_detalle and Id_Cantidad_Producto = $id_existencia_producto";
        $obj->query($qry_inventario_producto);
        
    }else{

        $qry_insert_inventario_actual = "insert into ds_tbl_cantidad_minima_producto (Id_Producto_Detalle, Cantidad_Minima) values ($id_producto_detalle, $wxistencia)";
    ...
    ?>
    <div id="resultado_input_cantidad_minima<?php echo $id_resultado; ?>" onclick="$(this).hide(); $('#input_alternativo_cantidad_minima<?php echo $id_resultado; ?>').show(); $('#input_cantidad_minima<?php echo $id_resultado; ?>').focus();"><?php echo $inventario_actual->Cantidad_Minima; //echo $resultado->Costo_Venta; ?></div>
