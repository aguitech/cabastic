    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Mes' <?php foreach($categorias_graph as $categorias_text){ ?>, '<?php echo $categorias_text; ?>'<?php } ?>],
          <?php foreach($meses as $mes_val): ?>
          <?php 
          //select * from ds_tbl_venta where month(Fecha_Venta) = 7   
          ?>
          ['<?php echo $mes_val; ?>'  <?php foreach($categorias_graph as $categorias_text){ ?>, 100<?php } ?>],
          <?php endforeach; ?>
