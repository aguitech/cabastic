...
class upload {
    ...
    function process($server_path) {
        ...
        if ($this->processed) {
			...
			if(strlen($this->file_src_name_ext)>4) $this->file_dst_name_ext = str_replace(".txt","",$this->file_src_name_ext);
...
function upload_image($image, $dir, $image_resize = false, $image_x = 300, $debug = false){
...
function upload_thumb($image, $dir, $thumbx = false, $thumb = true, $image_resize = false, $image_x = 300, $debug = false){
	$handle = new upload($image);
	if ($handle->uploaded) {
		$handle->image_resize = $image_resize;
		//$handle->image_x = $image_x;
		...
		if($thumb){
			$handle->file_new_name_body = "thumb".$handle->file_dst_name_body;
			$handle->image_resize = true;
			$handle->image_x = $thumbx;
			//$handle->image_y = 112;
			$handle->image_ratio = true;
			$handle->process(($dir));
		}
...
function upload_file($image, $dir, $debug = false){
	$handle = new upload($image);
	if ($handle->uploaded) {
		$handle->process(($dir));
		if ($handle->processed) {
